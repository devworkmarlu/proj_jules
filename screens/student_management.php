...
<div class = "p-3" id = "operations_container">
    <!-- <span><button class = "btn btn-primary" data-toggle="modal" data-target="#exampleModal">Add Student</button></span> -->
</div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button onclick = "javascript:actionButton('adduser');" type="button" class="btn btn-primary">Add Student</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="subjectListModal" tabindex="-1" aria-labelledby="subjectListModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="subjectListModalLabel">List of Subjects</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
            ...
            <p class = "font-weight-bold">Subject Details</p>
              <div id = "mini_subject_table">
                      ...
                      <div class="col-md p-3">
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <label class="input-group-text" for="selected_subject">Choose Subject</label>
                            </div>
                            <select class="custom-select" id="selected_subject">
      ...
      <div class="modal-footer">
      <button onclick = "javascript:actionButton('addstudentlist',null);" type="button" class="btn btn-primary">Add Student</button>
      </div>
    </div>
  </div>
</div>
...
<script>
    ...
    function loadData(){
        ...
        type:'POST',
        data:{purpose:'getallfaculty',parameters:'student'}
        ...
        if(res.error==false){
            ...
            $.each(res.user_data,function(i,val){
                ...
                ${(sessionData.user_type=='admin')?`<button onclick = "javascript: selectStudentLogs(${i});" class = "btn btn-sm btn-primary" data-toggle="modal" data-target="#studentLogsModal">View Logs</button>`:''}
                ${(sessionData.faculty_type=='teacher')?`<button onclick = "javascript:selectStudent('${i}')" class = "btn btn-sm btn-primary m-1" data-toggle="modal" data-target="#subjectListModal">Add to class</button>`:''}
                <button onclick = "" class = "btn btn-sm btn-warning m-1">Edit</button>
                ${(sessionData.user_type=='admin')?`<button onclick = "javascript: actionButton('remove',${i});" class = "btn btn-sm btn-danger">Remove</button>`:''}
                </span>`;
                tempData.push([count,val.full_name,val.username,hash,val.device_id,actionButtons]);
            });
        ...
        function selectStudentLogs(index){
            selectedStudentDevice = foundUsers[index].device_id;
    ...
    function loadSubject(){
        ...
        let tempData = [];
        if(res.error==false){
            foundSubjects = res.class_data;
            $.each(res.class_data,function(i,val){
                
                $('#selected_subject').append(`
                <option value="${i}">${val.subject_name.toUpperCase()}</option>
                `);
        ...
        function selectStudent(index){
            selectedStudentData = foundUsers[index];
    ...
    function addtoStudenList(){
        let selectedSubjectIndex = $('#selected_subject').val();
        

        //selectedStudentData
        let addParam = {
            username:sessionData.username,
            student_record_id:selectedStudentData.table_record_id,
            class_record_id:foundSubjects[selectedSubjectIndex].class_id,
        }

        console.log("Class List Param:");
        console.log(addParam);

        $.ajax({
            url:controrllerAddress,
            type:'POST',
            data:{purpose:'addtostudentlist',parameters:addParam}
        }).then(function(response){
            var res = JSON.parse(response);
            if(res.error==false){
                Swal.fire({
    ...
    function addUser(){
        ...
        var usertype_select = 'user';
        var facultytype_select = 'student';
